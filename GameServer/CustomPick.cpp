...
void CCustomPick::GetMove(LPOBJ lpObj, int index)
{
}
...
void CCustomPick::ItemGet(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < MAX_MAP_ITEM; n++)
	{
		...
		if (lpObj->PickupExc == 1 || lpObj->PickupSocket == 1 || lpObj->PickupSetItem == 1)
		{
			...
			if (lpObj->PickupExc == 1 && lpItem->IsExcItem() != 0)
			{
...
void CCustomPick::PickProc(LPOBJ lpObj) // OK
{
	...
	if (gMapManager.GetMapCustomPick(lpObj->Map) == 0)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index, 1, gMessage.GetTextMessage(754, lpObj->Lang));
		gNotice.GCNoticeSend(lpObj->Index, 1, gMessage.GetTextMessage(36, lpObj->Lang));
		return;
	}
...
bool CCustomPick::CommandPick(LPOBJ lpObj, char* arg)
{
	if (lpObj->Interface.use != 0 || lpObj->Teleport != 0 || lpObj->DieRegen != 0)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, gMessage.GetTextMessage(659, lpObj->Lang));
		return 0;
	}
	...
	if (strcmp(mode, "exc") == 0)
	{
		...
		{
			lpObj->PickupExc = 0;
			gNotice.GCNoticeSend(lpObj->Index, 1, gMessage.GetTextMessage(753, lpObj->Lang), mode);
		...
		{
			lpObj->PickupExc = 1;
			gNotice.GCNoticeSend(lpObj->Index, 1, gMessage.GetTextMessage(752, lpObj->Lang), mode);
